...
public class SwapAlphabetsUpperLower {

	@Test
	public void example1() {
		String inp = "BdAcbCaD"; // BbdDAacC
		System.out.println(swapAlphabets3(inp));
	}
	...
	private String swapAlphabets(String inp) {
		...
		for (int i = 0; i < inp.length(); i++) {
			...
			if (add)
				returnStr += Character.toString(inp.charAt(i));

			if (Character.isUpperCase(inp.charAt(i))) {
				add = set.add(Character.toLowerCase(inp.charAt(i)));
				if (add)
					returnStr += Character.toString(Character.toLowerCase(inp.charAt(i)));
			} else {
				add = set.add(Character.toUpperCase(inp.charAt(i)));
				if (add)
					returnStr += Character.toString(Character.toUpperCase(inp.charAt(i)));
			}
		}
	...
	private String swapAlphabets3(String inp) {
		...
		for (char c : inp.toCharArray()) {
			...
			if (list.contains((Character) chr)) {
				s += Character.toString(c)+Character.toString(chr) ;
